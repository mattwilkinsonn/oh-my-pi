...
- Fixed a brief black-flash flicker visible in four TUI render paths under tmux + ghostty. All four previously emitted `\x1b[2K` (erase entire line) BEFORE the new content, wrapped in BSU (`\x1b[?2026h…l`); when the BSU envelope split across PTY reads in tmux (~4 KB chunk size) the terminal erased the line before the content arrived, leaving a one-frame blank. The fix moves the erase AFTER the content (`\x1b[m\x1b[K` — SGR reset + erase from cursor to EOL) in every path: the differential render loop (per-keystroke, line-by-line), the trailing extra-line clear when content shrinks, the shrink-tail-only branch (`firstChanged >= newLines.length`, used when every change is at the end), and `viewportRefresh()` (fires when a markdown fence above the viewport changes mid-stream, e.g. `` ``` `` → `` ```python ``). Old content stays on screen until new content overwrites it character-by-character, so a BSU split now produces at worst a frame of stale trailing characters instead of a black bar — visually indistinguishable during keystroke- or streaming-paced repaint. The same fix transitively cures a vertical-collision bug where streaming markdown text overlapped Bash tool-call cards: `viewportRefresh()`'s BSU-split window was leaving `#hardwareCursorRow` desynced, and the next differential frame would write changed lines over rows that already held another component.
