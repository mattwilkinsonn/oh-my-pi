...
describe("hashline parser — suffix-op syntax", () => {
	...
	it("rejects op sigils written in prefix position (legacy syntax)", () => {
		expect(() => parseHashline(`↑${tag(1, "aaa")}\nold`)).toThrow(/unrecognized op/);
		expect(() => parseHashline(`↓${tag(1, "aaa")}\nold`)).toThrow(/unrecognized op/);
		expect(() => parseHashline(`→${tag(1, "aaa")}\nold`)).toThrow(/unrecognized op/);
	});

	it("rejects ranges with `..` separator (legacy syntax)", () => {
		// `..` is no longer the range separator; the line is treated as orphan
		// payload because `2yy..3yy→` does not match the new range pattern.
		expect(() => parseHashline(`${tag(2, "bbb")}..${tag(3, "ccc")}→\nBBB`)).toThrow(/payload line has no preceding/);
	});
	...
	it("leniently tolerates a trailing `|TEXT` body on anchors copied verbatim from read output", () => {
		const anchor = tag(2, "bbb");
		// Bare trailing `|`, full `|TEXT` body, and trailing decoration after a range.
		...
		expect(applyDiff(content, [`${anchor}|bbb↓`, pl("X")].join("\n"))).toBe("aaa\nbbb\nX\nccc");
		// Trailing `|TEXT` after the full range is also tolerated.
		expect(applyDiff(content, `${anchor}-${tag(3, "ccc")}|ccc→`)).toBe("aaa");
	});
	...
	it("anchor paste decoration `|TEXT` before the op is cosmetic; real payload comes after", () => {
		// `|bbb` between the anchor and the op is just paste decoration and is
		// discarded. Payload must come inline after the op or on the next lines.
		const anchor = tag(2, "bbb");
		const diff = [`${anchor}|bbb↓`, pl("X"), pl("Y")].join("\n");
		expect(applyDiff(content, diff)).toBe("aaa\nbbb\nX\nY\nccc");
		// No inline payload after the op and no follow-up: error.
		expect(() => parseHashline(`${anchor}|bbb↓`)).toThrow(/require at least one/);
	});

	it("treats `|TEXT` after BOF/EOF as cosmetic decoration", () => {
		expect(applyDiff(content, `BOF|head↓HEAD`)).toBe("HEAD\naaa\nbbb\nccc");
		expect(applyDiff(content, `EOF|tail↓TAIL`)).toBe("aaa\nbbb\nccc\nTAIL");
	});
});
